...
class UsersServices
{
    ...
    {
        return DB::transaction(function () use ($account, $isFakeUser) {
            ...
            $isAddress = (new Web3Api())->isAddress(Web3NetworkEnum::Ethereum->value, $address)['isAddress'] ?? false;
            if (!$isAddress)
                Err::Throw(__("Your wallet address is wrong"));

            $user = Users::where('address', $address)->lockForUpdate()->first();
            if ($user) {
                return $user;
            }
    ...
    public static function Export(mixed $query): StreamedResponse|string
    {
        ...
        $id = uniqid();
        return (new FastExcel(dateGenerator($query)))->download("用户表_$id.xlsx", function ($item) {
            return [
                'id' => $item->id,
                '昵称' => $item->nickname ?? '-',
                '状态' => $item->status,
