...
class BonusesServices
{
    ...
    {
        $exists = Bonuses::where('to_users_id', $user->id)
            ->where('type', UserBonusesTypeEnum::VerifyIdentity->name)
            ->lockForUpdate()
            ->exists();
        if ($exists)
            return;
...
//            'bonus_rate' => 0, #
     ...
     * @return void
     */
    public static function CreateByGoogle2FAVerify(Users $user): void
    {
        $exists = Bonuses::where('to_users_id', $user->id)
            ->where('type', UserBonusesTypeEnum::Google2FAVerify->name)
            ->lockForUpdate()
            ->exists();
        if ($exists)
            return;
        ...
        Bonuses::create([
            'from_users_id' => $user->id, #
            'to_users_id' => $user->id, #
            'type' => UserBonusesTypeEnum::Google2FAVerify->name, # 1:Referral / 2:PledgeProfit
//            'friend_bonus' => 0, #
//            'bonus_rate' => 0, #
            'bonus' => 20, #
            'status' => UserBonusesStatusEnum::Waiting->name, # 1:Waiting / 2:Failed / 3:Success
