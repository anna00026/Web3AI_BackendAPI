...
class StakingLogics
{
     ...
     * @throws Err
     */
    public static function CanStake(Users $user, float $amount, string $type = null): void
    {
        if (!$type)
...
//        $price = CoinServices::GetPrice($coin->symbol) * $amount;
     ...
     * @return array
     */
    public static function preStaking(): array
    {
        $minStaking = ConfigsServices::Get('other')['min_staking'];
        return [
            'min' => $minStaking,
     ...
     * @throws Exception
     */
    public static function CreateStaking(string $network, Users $user, Coins $coin, float $amount, string $hash): void
    {
        CommonHelper::Trans(function () use ($network, $user, $coin, $amount, $hash) {
...
//                'staking_ended_at' => '', #
            ...
            $coinAddress = CoinServices::GetCoinByNetworkAndSymbol($pending->coin_network, $coin->symbol);
...
//                'message' => '', # 回傳訊息
     ...
     * @throws Err
     */
    public static function StakingCallback(Web3Transactions $web3, TxModel $hashData): void
    {
        try {
            DB::beginTransaction();
            ...
            // update web3
            $web3->coin_amount = $hashData->value;
            ...
            $pending->balance = $hashData->value;
