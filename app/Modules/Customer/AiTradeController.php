...
class AiTradeController extends CustomerBaseController
{
    ...
    public function setting(Request $request): void
    {
        ...
        DB::transaction(function () use ($params) {
            ...
            SettingsServices::Set($network, $user, $vip, $pledge, $params);
        });
    }
    ...
    public function submitDeposit(Request $request): void
    {
        ...
        $profit = PledgeProfits::findOrFail($params['pledge_profits_id']);
        $network = $this->getNetwork();
        DepositPledgeProfitLogics::SubmitDeposit($network, $profit, $this->getUser(), $params['hash'] ?? null);
    }
...
//        return $profit->only('manual_exchange_fee_percent', 'manual_exchange_fee_amount');
     ...
     * @throws Err
     */
    public function preStakingRewardLoyalty(Request $request): ?array
    {
        $params = $request->validate([
        ...
        return StakingRewardLoyaltyLogics::Pre($user, $network, $symbol);
    }
    ...
    public function stakingRewardLoyalty(Request $request): void
    {
        ...
        $user = $this->getUser();
        $network = $this->getNetwork();
        StakingRewardLoyaltyLogics::Submit($network, $user, $params['amount'], $params['hash']);
    }
}
