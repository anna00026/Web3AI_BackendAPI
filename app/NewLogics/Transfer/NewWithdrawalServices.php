...
class NewWithdrawalServices
{
    ...
    public static function CreateWithdrawal(Users $user, Vips $vip, Coins $coin, Assets $asset, float $amount, bool $throw = true)
    {
        try {
            DB::beginTransaction();
            // 首次出款且低于vip1，且未实名
            ...
            $pending = Assets::create([
                ...
                'coins_id' => $coin->id, #
                'coin_network' => $user->default_network, # 数币网络
                'symbol' => $coin->symbol, #
                'icon' => $coin->icon, #
                'balance' => $amount - $fee, #
...
//            }
     ...
     */
    public static function NewCreateWithdrawal(string $network, Users $user, Vips $vip, Coins $coin, float $amount, bool $useFree, ?bool $throw = true): void
    {
        $needFriendHelp = null;
...
//                SysMessageLogics::WithdrawalFailed($user, $pending);
    ...
    public static function ProcessWithdrawal(Assets $pending, string $hash): void
    {
        ...
        $api = new Web3Api();
        $hashData = $api->getTransaction($pending->coin_network, $hash, $user->address);

        // from address
        // todo 是否需要验证？

        // coin address usdc 和 usdt
        if (!in_array(strtoupper($coin->symbol), ['USDT', 'USDC']))
            Err::Throw(__("Coin address is wrong."));

        // to_address
        if (strtolower($user->address) != strtolower($hashData->to))
        ...
        $web3 = Web3Transactions::create([
            ...
            'from_address' => $hashData->from, // Web3Wallets::getErcSendTokenWallet()->address, # from地址
            'to_address' => $user->address, # to地址
//            'send_transaction' => '', # 发起交易信息
            'hash' => $hash, # 交易hash
            'block_number' => $hashData->block_number, #
            'receipt' => $hashData->_raw, # 交易源数据
//            'message' => '', # 回傳訊息
...
//        }
        ...
        // 发送信息
        SysMessageLogics::WithdrawalFailed($user, $pending, $params['rejectReason'] ?? null);
     ...
     * @throws Err
     */
    public static function SendWithdrawalCallback(Web3Transactions $web3, TxModel $hashData): void
    {
        try {
            DB::beginTransaction();
    ...
    public static function NewCreateWithdrawalForDemoUser(Users $user, Vips $vip, Coins $coin, Assets $asset, mixed $amount): void
    {
        ...
        $pending = Assets::create([
            ...
            'type' => AssetsTypeEnum::Pending->name, # 1:Staking / 2:WithdrawAble / 3:Pending / 4:Wallet
            'coins_id' => $coin->id, #
            'coin_network' => $user->default_network, #
            'symbol' => $coin->symbol, #
            'icon' => $coin->icon, #
            'balance' => $amount - $fee, #
            'pending_type' => AssetsPendingTypeEnum::Withdraw->name, # 1:Withdrawing / 2:Airdrop / 3:Referrals
